...
const Index = () => {
  ...
  return (
    ...
    <>
      ...
      <title>{hotelInfo.fullName} | Luxury Accommodation in Machakos, Kenya</title>

      <meta name="description" content={`${hotelInfo.description} Book your stay at ${hotelInfo.fullName} - rooms starting from $25/night.`} />
...
-- Create rooms table for dynamic room data

-- Source: Booking.com, Hotels.com, Agoda pricing data
...
CREATE TABLE public.rooms (
  ...
  price_usd_min DECIMAL(10,2) NOT NULL,

  price_usd_max DECIMAL(10,2) NOT NULL,

  price_kes_min DECIMAL(10,2),

  price_kes_max DECIMAL(10,2),
...
-- Create bookings table for direct reservations
...
-- Reviews are rated out of 5 stars
...
-- Create event inquiries table for seminars/conferences
...
-- Public read access for rooms (anyone can view rooms)
...
-- Public can create bookings (no auth required for hotel booking)
...
-- Anyone can submit event inquiries
...
EXECUTE FUNCTION public.update_updated_at_column();-- Fix CRITICAL: Server-side price calculation to prevent booking price fraud

-- Users could previously manipulate total_price_kes via browser devtools



-- 1. Create function to calculate booking price server-side
...
BEGIN
  ...
  SELECT price_kes_min INTO v_room_price

  FROM public.rooms

  WHERE id = p_room_id AND is_available = true;

  

  IF v_room_price IS NULL THEN

    RAISE EXCEPTION 'Invalid or unavailable room';

  END IF;

  

  RETURN v_nights * v_room_price;
...
-- 2. Create trigger function to automatically set price on insert
...
BEGIN
  ...
  -- Always calculate price server-side, ignoring any client-provided value

  NEW.total_price_kes := public.calculate_booking_price(
...
-- 3. Create trigger to enforce server-side price calculation
...
EXECUTE FUNCTION public.set_booking_price();



-- 4. Add input validation constraints to bookings table

ALTER TABLE public.bookings 

  ADD CONSTRAINT guest_name_length CHECK (length(guest_name) BETWEEN 2 AND 100),

  ADD CONSTRAINT guest_email_format CHECK (guest_email ~* '^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$'),

  ADD CONSTRAINT guest_email_length CHECK (length(guest_email) <= 255),

  ADD CONSTRAINT guest_phone_length CHECK (length(guest_phone) BETWEEN 7 AND 20),

  ADD CONSTRAINT special_requests_length CHECK (special_requests IS NULL OR length(special_requests) <= 1000),

  ADD CONSTRAINT valid_date_range CHECK (check_out_date > check_in_date),

  ADD CONSTRAINT reasonable_stay CHECK (check_out_date <= check_in_date + INTERVAL '90 days'),

  ADD CONSTRAINT num_guests_range CHECK (num_guests >= 1 AND num_guests <= 10),

  ADD CONSTRAINT total_price_positive CHECK (total_price_kes IS NULL OR total_price_kes > 0);



-- 5. Add input validation constraints to event_inquiries table

ALTER TABLE public.event_inquiries

  ADD CONSTRAINT contact_name_length CHECK (length(contact_name) BETWEEN 2 AND 100),

  ADD CONSTRAINT contact_email_format CHECK (contact_email ~* '^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$'),

  ADD CONSTRAINT contact_phone_length CHECK (length(contact_phone) BETWEEN 7 AND 20),

  ADD CONSTRAINT message_length CHECK (message IS NULL OR length(message) <= 2000),

  ADD CONSTRAINT attendees_range CHECK (expected_attendees IS NULL OR (expected_attendees >= 1 AND expected_attendees <= 1000));



-- 6. Add input validation constraints to reviews table

ALTER TABLE public.reviews

  ADD CONSTRAINT review_guest_name_length CHECK (length(guest_name) BETWEEN 2 AND 100),

  ADD CONSTRAINT comment_length CHECK (length(comment) BETWEEN 10 AND 2000),

  ADD CONSTRAINT rating_range CHECK (rating >= 1 AND rating <= 5),

  ADD CONSTRAINT guest_location_length CHECK (guest_location IS NULL OR length(guest_location) <= 100);project_id = "syknqsyovajjaoaixdwr"VITE_SUPABASE_PROJECT_ID="syknqsyovajjaoaixdwr"

VITE_SUPABASE_PUBLISHABLE_KEY="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InN5a25xc3lvdmFqamFvYWl4ZHdyIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjY0MDYwODUsImV4cCI6MjA4MTk4MjA4NX0.pmeqOEYh3UVQ8P1dodjtcvyG61nKFYTVgWYY3grw1O0"

VITE_SUPABASE_URL="https://syknqsyovajjaoaixdwr.supabase.co"
...

{
  ...
  "packages": {
    ...
    "@supabase/functions-js": ["@supabase/functions-js@2.89.0", "", { "dependencies": { "tslib": "2.8.1" } }, "sha512-XEueaC5gMe5NufNYfBh9kPwJlP5M2f+Ogr8rvhmRDAZNHgY6mI35RCkYDijd92pMcNM7g8pUUJov93UGUnqfyw=="],



    "@supabase/postgrest-js": ["@supabase/postgrest-js@2.89.0", "", { "dependencies": { "tslib": "2.8.1" } }, "sha512-/b0fKrxV9i7RNOEXMno/I1862RsYhuUo+Q6m6z3ar1f4ulTMXnDfv0y4YYxK2POcgrOXQOgKYQx1eArybyNvtg=="],



    "@supabase/realtime-js": ["@supabase/realtime-js@2.89.0", "", { "dependencies": { "@types/phoenix": "^1.6.6", "@types/ws": "^8.18.1", "tslib": "2.8.1", "ws": "^8.18.2" } }, "sha512-aMOvfDb2a52u6PX6jrrjvACHXGV3zsOlWRzZsTIOAJa0hOVvRp01AwC1+nLTGUzxzezejrYeCX+KnnM1xHdl+w=="],



    "@supabase/storage-js": ["@supabase/storage-js@2.89.0", "", { "dependencies": { "iceberg-js": "^0.8.1", "tslib": "2.8.1" } }, "sha512-6zKcXofk/M/4Eato7iqpRh+B+vnxeiTumCIP+Tz26xEqIiywzD9JxHq+udRrDuv6hXE+pmetvJd8n5wcf4MFRQ=="],



    "@supabase/supabase-js": ["@supabase/supabase-js@2.89.0", "", { "dependencies": { "@supabase/auth-js": "2.89.0", "@supabase/functions-js": "2.89.0", "@supabase/postgrest-js": "2.89.0", "@supabase/realtime-js": "2.89.0", "@supabase/storage-js": "2.89.0" } }, "sha512-KlaRwSfFA0fD73PYVMHj5/iXFtQGCcX7PSx0FdQwYEEw9b2wqM7GxadY+5YwcmuEhalmjFB/YvqaoNVF+sWUlg=="],
...
<html lang="en">
  ...
  <head>
    ...
    <meta property="og:title" content="hfbf">

  <meta name="twitter:title" content="hfbf">
